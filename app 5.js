...
function drawRipples() {
  for (let i = ripples.length - 1; i >= 0; i--) {
    ...
    // Soft fuzzy rings — radial gradient bands
    for (let k = 0; k < 3; k++) {
      const kr = rp.r - k * 32;
      if (kr <= 0) continue;
      ...
      const outer = kr + w;
      if (outer <= 0) continue;
      // inner must be strictly less than outer and >= 0
      const inner = Math.min(Math.max(0, kr - w), outer - 1);
      const band = ctx.createRadialGradient(rp.x, rp.y, inner, rp.x, rp.y, outer);
      band.addColorStop(0,   'rgba(255,255,255,0)');
      band.addColorStop(0.38, `rgba(255,255,255,${(ka * 0.55).toFixed(3)})`);
      band.addColorStop(0.5,  `rgba(255,255,255,${ka.toFixed(3)})`);
      band.addColorStop(0.62, `rgba(255,255,255,${(ka * 0.55).toFixed(3)})`);
