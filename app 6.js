...
function drawRipples() {
  ...
  for (var i = ripples.length - 1; i >= 0; i--) {
    ...
    for (var k = 0; k < 3; k++) {
      var kr = prog - k * 34;
      if (kr < 2) continue;
      var a = fade * (1 - k * 0.28) * 0.55;
      ctx.beginPath();
      ctx.arc(rp.x, rp.y, kr, 0, Math.PI * 2);
      ctx.strokeStyle = 'white';
      ctx.globalAlpha = a * 0.35;
      ctx.lineWidth = 18 + k * 6;
      ctx.stroke();
    ...
    if (age < BLOOM) {
      var t = age / BLOOM;
      var env = t < 0.35 ? t / 0.35 : 1 - (t - 0.35) / 0.65;
      var bA = env * 0.18;
      var bR = Math.max(2, 40 + t * 120);
      var g = ctx.createRadialGradient(rp.x, rp.y, 0, rp.x, rp.y, bR);
