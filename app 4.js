...
function drawRipples() {
  for (let i = ripples.length - 1; i >= 0; i--) {
    ...
    // Soft fuzzy rings — drawn as radial gradient bands instead of strokes
    for (let k = 0; k < 3; k++) {
      const kr = rp.r - k * 32;
      if (kr <= 0) continue;
      const ka = rp.alpha * (1 - k * 0.32);
      const w = 18 + k * 8; // band width — wider = softer
      const inner = Math.max(0, kr - w);
      const outer = kr + w;
      const band = ctx.createRadialGradient(rp.x, rp.y, inner, rp.x, rp.y, outer);
      band.addColorStop(0,   'rgba(255,255,255,0)');
      band.addColorStop(0.4, `rgba(255,255,255,${(ka * 0.6).toFixed(3)})`);
      band.addColorStop(0.5, `rgba(255,255,255,${ka.toFixed(3)})`);
      band.addColorStop(0.6, `rgba(255,255,255,${(ka * 0.6).toFixed(3)})`);
    ...
    // Central glow — soft, slow bloom over 1.4s
    const BLOOM_DUR = 1.4;
    if (age < BLOOM_DUR) {
      const t = age / BLOOM_DUR;
      // ease-in-out: rise then fall
      const env = t < 0.35
        ? t / 0.35                        // fade in
        : 1 - (t - 0.35) / 0.65;         // slow fade out
      const bloomA = env * 0.22;
      const bloomR = 40 + t * 120;        // glow expands gently
      const flash = ctx.createRadialGradient(rp.x, rp.y, 0, rp.x, rp.y, bloomR);
      flash.addColorStop(0,   `rgba(255,255,255,${bloomA.toFixed(3)})`);
      flash.addColorStop(0.5, `rgba(220,240,255,${(bloomA * 0.5).toFixed(3)})`);
      flash.addColorStop(1,   'rgba(255,255,255,0)');
      ctx.beginPath();
      ctx.arc(rp.x, rp.y, bloomR, 0, Math.PI * 2);
      ctx.fillStyle = flash;
      ctx.fill();
    }
  }
